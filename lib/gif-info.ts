...
export function gifInfo(bytes: Uint8Array) {
  const text = String.fromCharCode(...bytes.slice(0, 6));
  if (!['GIF87a', 'GIF89a'].includes(text) || bytes.length < 14) throw Error('Invalid GIF.');
  const word = (offset: number) => bytes[offset] | (bytes[offset + 1] << 8);
  const width = word(6), height = word(8);
  let cursor = 13, frames = 0;
  ...
  while (cursor < bytes.length) {
    ...
    if (marker === 0x3b) {
      if (!frames || !width || !height) throw Error('Invalid GIF.');
    ...
    if (marker !== 0x2c) {
      if (frames > 0 && width && height) return { width, height, frames };
      throw Error('Corrupted GIF.');
    }
    ...
    subblocks();
    frames++;
    if (frames > GIF_MAX_FRAMES || width * height * frames > GIF_MAX_TOTAL_PIXELS) {
      throw Error('GIF is too large or has too many frames. Please upload a smaller file.');
    }
  }

  if (frames > 0 && width && height) {
    return { width, height, frames };
  }
  throw Error('Incomplete GIF.');
}
