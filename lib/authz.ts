...
export async function requireSettingsUser(returnTo='/settings') {
  const principal=await getPrincipal();
  if(!principal||principal.status!=='ACTIVE')redirect(`/signin?callbackUrl=${encodeURIComponent(returnTo)}`);
  return principal;
}
