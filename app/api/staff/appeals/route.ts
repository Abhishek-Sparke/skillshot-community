...
export async function PATCH(request: Request) {
  ...
  if(action==='ACCEPT'&&appeal.target_type==='SKILLSHOT'&&auth.principal.permissions.includes('skillshots.restore'))await sql.query(`UPDATE posts SET status='VISIBLE' WHERE id=$1`,[appeal.target_id]);
