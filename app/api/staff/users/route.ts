...
export async function GET(request: Request) {
  ...
  const users = await (await getReadyDb()).query(`SELECT id,display_name,username,role,status,created_at,avatar_url,creator_rank FROM users WHERE $1='' OR username ILIKE $2 OR display_name ILIKE $2 ORDER BY (CASE WHEN username ILIKE $3 THEN 0 WHEN display_name ILIKE $3 THEN 1 ELSE 2 END), username ASC, created_at DESC LIMIT 60`, [q, `%${q}%`, `${q}%`]);
