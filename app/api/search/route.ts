...
export async function GET(request:Request) {
  ...
  try {
    ...
    const [people,shots,categories,skills]=await Promise.all([
      filter.type==='shots'?Promise.resolve([]):sql.query(`SELECT u.username,u.display_name,u.skills,u.role,u.creator_rank,(u.avatar_url IS NOT NULL) has_avatar,
      ...
      filter.type==='people'?Promise.resolve([]):sql.query(`SELECT p.id,p.title,p.skills,p.category,p.image_width,p.image_height,u.display_name,u.username,u.role,u.creator_rank,
    ...
    return Response.json({people:people.slice(0,limit).map(p=>({...p,creatorRank:p.creator_rank||'NEWCOMER',role:normalizeRole(p.role),avatarUrl:p.has_avatar?'/api/avatars/'+encodeURIComponent(p.username):''})),shots:shots.slice(0,limit).map(p=>({...p,creatorRank:p.creator_rank||'NEWCOMER',role:normalizeRole(p.role),imageUrl:'/api/images/'+p.id+'?variant=thumbnail'})),peopleMore:people.length>limit,shotsMore:shots.length>limit,categories:categories.map(c=>c.category),skills:skills.map(s=>s.skill)},{headers:{'Cache-Control':'private, no-cache'}});
