...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await sql.query(`SELECT c.id,c.user_id,c.body,c.created_at,u.display_name,u.username,u.email,u.role FROM comments c JOIN users u ON u.id=c.user_id WHERE c.post_id=$1 ORDER BY c.created_at ASC`, [id]);
