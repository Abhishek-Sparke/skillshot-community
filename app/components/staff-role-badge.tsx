...
const STAFF_ROLE_LABELS: Partial<Record<UserRole, string>> = {
  OWNER: 'Owner',
  ADMIN: 'Admin',
...
export default function StaffRoleBadge({
  ...
  const label = STAFF_ROLE_LABELS[role as UserRole];
  ...
  const badge = (
    ...
    >
      <span className="roleBadgeIcon">
        <RoleIcon role={role as UserRole} />
      </span>
    </IconBadge>
  );

  return showLabel ? (
    <span className="roleBadgeLabel">
      {badge}
      <b>{label}</b>
    </span>
  ) : (
