...
export default function ShotDetail({id}:{id:string}) {
  ...
  return <main className="detailPage shell">
    ...
    <div className="detailLayout"><section className="detailVisual"><button type="button" className="detailImageButton" onClick={()=>setViewerOpen(true)} aria-label={'Open fullscreen preview of '+post.title}><img className="detailImage" src={post.imageUrl} alt={post.title} width={post.imageWidth} height={post.imageHeight}/><span>↗ Preview</span></button></section>
      <aside className="detailSidebar"><div className="detailHeading"><div><p className="eyebrow">{post.category||'SKILLSHOT'}</p><h1>{post.title}</h1></div></div>
        <Link className="detailCreator" href={'/users/'+encodeURIComponent(post.username)}><span className="detailAvatar">{post.avatarUrl?<img src={post.avatarUrl} alt="" loading="lazy"/>:<i>{initials(post.author)}</i>}</span><span className="detailAuthorDetails"><b>{post.author} <RoleBadge role={post.authorRole}/></b><small className="detailHandle">{'@' + post.username}</small></span></Link>
        ...
        <div className="detailActions">
          ...
          <button type="button" className={'detailSaveButton '+(post.viewerSaved?'saved':'')} onClick={toggleSave}>🔖 {post.viewerSaved?'Saved':'Save'}</button>
          <button type="button" className="detailShareButton" onClick={shareSkillshot}>↗ {sharedLabel}</button>
