...
import TrustedReview from './trusted-review';
...
type Appeal={id:string;username:string;target_type:string;reason:string;explanation:string;created_at:string};
...
export default function StaffPanel({focus='all'}:{title?:string;focus?:'all'|'moderation'|'reports'|'skillshots'|'comments'|'trusted'}) {
  ...
  return <div className="staffPageBody">
    ...
    {focus==='reports'?<ReportCases/>:['moderation','skillshots','comments'].includes(focus)&&<ModerationQueue focus={focus}/>}
    {(focus==='all'||focus==='trusted')&&user.permissions.includes('trusted_contributor.review')&&<TrustedReview/>}
    {(focus==='all'||focus==='reports')&&<section className="staffSection"><h2>Appeals & support requests</h2>{!data?(error?<StaffError retry={()=>setRevision(v=>v+1)}/>:<StaffSkeleton/>):!data.appeals.length?<StaffEmpty title="No pending appeals"/>:<div className="staffTable">{data.appeals.map(item=><article key={item.id}><div><b>{item.reason}</b><p>{item.explanation}</p><span>@{item.username} · {item.target_type}</span><small>{new Date(item.created_at).toLocaleString()}</small></div>{user.permissions.includes('reports.resolve')&&<div>{(item.target_type!=='SKILLSHOT'||user.permissions.includes('skillshots.restore'))&&<button disabled={!!busy} onClick={()=>review('appeals',item.id,'ACCEPT')}>{item.target_type==='SUPPORT'?'Resolve':'Accept'}</button>}<button disabled={!!busy} onClick={()=>review('appeals',item.id,'REJECT')}>Reject</button><button disabled={!!busy} onClick={()=>review('appeals',item.id,'REVIEW')}>Mark under review</button></div>}</article>)}</div>}</section>}
