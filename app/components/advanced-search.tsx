...
type Person={username:string;display_name:string;skills:string[];role:UserRole;avatarUrl:string};
type Shot={id:string;title:string;display_name:string;username:string;skills:string[];role:UserRole;imageUrl:string;image_width:number;image_height:number;likes:number;comments:number};
...
useEffect(()=>{if(!mobileFilters)return;const node=filterPanel.current;if(!node)return;const previous=document.activeElement as HTMLElement|null,opener=filterButton.current;const focusable=()=>Array.from(node.querySelectorAll<HTMLElement>('button:not([disabled]),input:not([disabled]),select:not([disabled])'));focusable()[0]?.focus();const keyboard=(event:KeyboardEvent)=>{if(event.key==='Escape'){setMobileFilters(false);filterButton.current?.focus();return;}if(event.key!=='Tab')return;const items=focusable(),first=items[0],last=items.at(-1);if(!first||!last)return;if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}};document.addEventListener('keydown',keyboard);const overflow=document.body.style.overflow;document.body.style.overflow='hidden';return()=>{document.removeEventListener('keydown',keyboard);document.body.style.overflow=overflow;if(previous&&!opener?.contains(document.activeElement))previous.focus();};},[mobileFilters]);
  ...
  return <section className="searchWorkspace">
    ...
    {error?<div className="emptyFeed" role="alert"><h3>{error}</h3><button onClick={()=>{setLoading(true);setRevision(v=>v+1);}}>Try again</button></div>:!loading&&results&&!((people&&results.people.length>0)||(shots&&results.shots.length>0))?<div className="emptyFeed"><span>✦</span><h3>No search results</h3><p>Nothing matched your search.</p></div>:<div aria-busy={loading}>{loading&&<p role="status">Searching…</p>}{people&&<section><h2>People</h2><div className="creatorResults">{results?.people.map(person=><article key={person.username}><Link href={'/users/'+encodeURIComponent(person.username)}>{person.avatarUrl&&<Image unoptimized src={person.avatarUrl} width={48} height={48} alt=""/>}<b>{person.display_name}</b></Link><RoleBadge role={person.role}/><p>{person.skills?.slice(0,3).join(' · ')}</p><Link href={'/users/'+encodeURIComponent(person.username)}>View profile →</Link></article>)}</div>{!loading&&!results?.people.length&&<p>No people found. Try another name or skill.</p>}</section>}
      {shots&&<section><h2>Skillshots</h2><div className="searchResults masonryGrid">{results?.shots.map(shot=><article key={shot.id}><Link className="searchImage" href={'/shots/'+shot.id}><Image unoptimized src={shot.imageUrl} width={shot.image_width||640} height={shot.image_height||480} alt={shot.title+' by '+shot.display_name} loading="lazy"/></Link><div><h3><Link href={'/shots/'+shot.id}>{shot.title}</Link></h3><p><Link href={'/users/'+encodeURIComponent(shot.username)}>{shot.display_name}</Link> <RoleBadge role={shot.role}/></p><p>{shot.skills?.slice(0,3).join(' · ')}</p><small>♡ {shot.likes} · ◌ {shot.comments}</small></div></article>)}</div>{!loading&&!results?.shots.length&&<p>No Skillshots found. Try people, skills or categories.</p>}</section>}
