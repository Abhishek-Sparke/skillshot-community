  ...
  return <>
    ...
    {view&&createPortal(<dialog ref={dialog} className="settingsDialog" aria-labelledby="settings-menu-title" onCancel={()=>setView(null)} onClick={event=>{if(event.target===event.currentTarget)setView(null);}}>
      ...
      {view==='account'?<div className="settingsMenuRows">
        ...
        {logout}
      </div>:sections.map(([title,items])=><section key={title}><h3>{title}</h3><div className="settingsMenuRows">{items.map(([label,href])=><Link key={href} href={href} onClick={()=>setView(null)}>{label}<span aria-hidden="true">›</span></Link>)}</div></section>)}
      </div>
