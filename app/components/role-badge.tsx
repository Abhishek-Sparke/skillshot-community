...
export default function RoleBadge({ role, variant = 'compact' }: { role: UserRole; variant?: 'profile'|'compact' }) {
  ...
  return <span className={`roleBadge role${role} ${variant}`} aria-label={tooltip} title={tooltip} data-tooltip={tooltip} role="img" tabIndex={0}><span className="roleBadgeIcon"><RoleIcon role={role}/></span>{variant === 'profile' && <b>{label}</b>}</span>;
}
