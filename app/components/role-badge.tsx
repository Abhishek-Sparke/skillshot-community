...
export default function RoleBadge({ role, variant = 'compact', showLabel = variant === 'profile' }: { role: UserRole; variant?: 'profile'|'compact'; showLabel?: boolean }) {
  const label = labels[role];
  if (!label) return null;
  const tooltip=`${label} — Staff Role`;
  const badge = <IconBadge className={`roleBadge role${role} ${variant}`} size={variant} tooltip={tooltip}><span className="roleBadgeIcon"><RoleIcon role={role}/></span></IconBadge>;
