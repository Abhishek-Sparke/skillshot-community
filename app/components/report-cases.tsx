...
export default function ReportCases() {
  ...
  return <section className="moderationWorkspace" aria-label="Report cases">
    ...
    {error===key?<StaffError retry={()=>setRevision(v=>v+1)}/>:data?.key!==key?<StaffSkeleton/>:!data.items.length?<StaffEmpty title={filters.mine==='1'?'No cases assigned to you.':filters.status==='PENDING'?'All clear — no reports need review.':'No cases match these filters.'}/>:<>
      <div className="moderationGrid">{data.items.map(item=><article className="moderationCard" key={item.id}><div className="moderationCardBody"><div className="staffCardTop"><span className="eyebrow">{item.parent_id?'REPLY':item.target_type} · R-{item.number}</span><StaffStatus value={item.status}/></div><h2>{item.title||item.display_name||'Content unavailable'}</h2>{item.username&&<p>@{item.username}</p>}{item.comment_body&&<blockquote>{item.comment_body.slice(0,180)}</blockquote>}<p>{Number(item.report_count)} reports · {item.priority==='HIGH'?'High priority':'Normal priority'}</p><p>{item.reasons.map(reason=>`${reason.count} × ${readable(reason.category)}`).join(' · ')}</p><p className="staffHint">{item.automatic?'Automatic flag · signal for human review': 'User report'}<br/>{item.reporter?`Reported by @${item.reporter}`:''}<br/>{new Date(item.created_at).toLocaleString()}<br/>{item.assigned_username?`Assigned to @${item.assigned_username}`:'Unassigned'}</p><Link className="staffDetailLink" prefetch={false} href={`${root}/reports/${item.id}`}>Review Case →</Link></div></article>)}</div>
