...
type Item = { id: string; source: string; target_type: string; target_id: string; category: string; severity: string; status: string; created_at: string; original_created_at?: string; username: string; display_name: string; has_avatar: boolean; bio?: string; title?: string; description?: string; comment_body?: string; post_id?: string; parent_id?: string; context_title?: string; current_status?: string; report_count: number; reports: Report[]; history: { action: string; username: string; created_at: string }[] };
...
function Preview({ item }: { item: Item }) {
  ...
  if (item.target_type === 'COMMENT') return <div className="moderationQuote"><span className="eyebrow">{item.parent_id ? 'REPLY' : 'COMMENT'}</span><blockquote>{item.comment_body || 'This comment is no longer available.'}</blockquote>{item.post_id && <Link href={`/shots/${encodeURIComponent(item.post_id)}`}>On: {item.context_title || 'View Skillshot'} ↗</Link>}</div>;
